...
本项目所有部分都经过各目录下的`test_test.go`文件几百次测试，由于不确定性的存在，开发期间一个错误可能一两百次测试才会复现，所以完成项目的过程中打印了大量的调试日志方便找出bug，由于大量异步协程、分布式节点的存在，所以开发过程中有很多意想不到的corner case，基本都在代码注释中标注了。
...
### 客户端
客户端的每一个请求，都通过rpc包的Call函数，不断循环尝试直到成功，尝试失败返回若为ErrWrongGroup，则会Query新的分片配置重试，其他情况尝试失败则会Group内换一台机器重试，换了一轮还不行则Query新的配置重试。该部分实现在`src\shardctrler\client.go`中。

之所以Group内切换服务器尝试一轮而不是一直重试是为了避免旧config中的group已经shutdown，但是call失败会一直在本group切换机器，同样会call失败，一直陷入for循环，这里call一整轮都失败了的话就跳出获取新config。